...
# --- Render "Timed Out" ስህተትን ለመከላከል (Keep Alive) ---
flask_app = Flask('')
@flask_app.route('/')
...
STRINGS = {
    ...
    "en": {
        ...
        "receipt_sent": "✅ Receipt sent! Admin will notify you.",
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    home_cancel_words = []
    for l in STRINGS:
        home_cancel_words.extend([STRINGS[l]["home"], STRINGS[l]["cancel"]])

    if text in home_cancel_words:
        ctx.user_data.clear()
        ctx.user_data["lang"] = lang
        title = await db.get_setting("lottery_title")
...
async def admin_panel_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # እዚህ ጋር እንደፈለጉ የአድሚን ዝርዝሮችን መጨመር ይችላሉ
    await query.message.reply_text("🛠 Admin Panel is active.")

async def post_init(app):
    await db.init_db()

def main():
    # Flask ሰርቨርን በሌላ Thread አስጀምር (Render Keep-Alive)
    ...
    # Conflict ለመከላከል drop_pending_updates=True
